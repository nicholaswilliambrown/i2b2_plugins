// ================================================================================================== //
i2b2.CRC.cfg.msgs.getFamilialRelationships = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\r'+
...
'		  xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">\n'+
			'	<analysis_definition>\n' +
			'		<analysis_plugin_name>Familial_Relationship</analysis_plugin_name>\n' +
			'		<version>1.0</version>\n' +
			'		<crc_analysis_input_param name="ONT request">\n' +
			'			<param type="int" column="resultInstanceID1">{{{resultInstanceID1}}}</param>\n' +
			'			<param type="int" column="resultInstanceID2">{{{resultInstanceID2}}}</param>\n' +
			'			<param type="string" column="relationshipType">{{{relationshipType}}}</param>\n' +			
			'			<param type="string" column="queryName">{{{queryName}}}</param>\n' +		
...
i2b2.CRC.cfg.parsers.getFamilialRelationships = function(){
	if (!this.error) {
		this.model = {
			patients: [],
			events: [],
...
i2b2.CRC.ajax._addFunctionCall(	"getFamilialRelationships",
								"{{{URL}}}request",
								i2b2.CRC.cfg.msgs.getFamilialRelationships,
								["HSD_Request"],
								i2b2.CRC.cfg.parsers.getFamilialRelationships);			